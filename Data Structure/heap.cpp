// heap.cpp   Signed: Seungye Bae   Student Number: 21600326
...
void grow(heap p, int key) {
	...
	if (full(p)) reserve(p, capacity(p)*2);
	p->nodes[++p->N] = key;
	swim(p, p->N);
...
void trim(heap p) {
	...
	if (p->N == (capacity(p)/4) - 1) reserve(p, capacity(p)/2);
...
void growN(heap p, int count, bool heapOrdered) {
	...
	int* arr = new (nothrow) int[count];
	assert(arr != nullptr);
	randomN(max, count, arr);

	for(int i=0; i < count; i++) {
		insertFunc(p, arr[i]);
		DPRINT(cout << heapprint(p) << endl;);
	}
...
void trimN(heap p, int count, bool heapOrdered) {
	...
	void(*insertFunc)(heap h) = heapOrdered ? trim : trimCBT;
